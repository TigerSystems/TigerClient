...
public class AdvancedFontManager {
	...
	public Map<ResourceLocation, List<GlyphProvider>> prepare(ResourceManager p_95024_, ProfilerFiller p_95025_) {
        p_95025_.startTick();
              ...
              for(Resource resource : p_95024_.getResources(resourcelocation)) {
                 ...
                 try {
                          ...
                          for(int i = jsonarray.size() - 1; i >= 0; --i) {
                             ...
                             try {
                                ...
                                p_95025_.pop();
                            	 ...
                            	 AdvancedFontManager.LOGGER.warn("Unable to read definition '{}' in {} in resourcepack: '{}': {}", resourcelocation1, "fonts.json", resource.getSourceName(), runtimeexception.getMessage());
                             }
                          }
                    ...
                    if (inputstream != null) {
                	 ...
                	 AdvancedFontManager.LOGGER.warn("Unable to load font '{}' in {} in resourcepack: '{}': {}", resourcelocation1, "fonts.json", resource.getSourceName(), runtimeexception1.getMessage());
                 }

                 p_95025_.pop();
              }
           } catch (IOException ioexception) {
        	   AdvancedFontManager.LOGGER.warn("Unable to load font '{}' in {}: {}", resourcelocation1, "fonts.json", ioexception.getMessage());
           }
     ...
     public void apply(Map<ResourceLocation, List<GlyphProvider>> p_95036_, ResourceManager p_95037_, ProfilerFiller p_95038_) {
        p_95038_.startTick();
        p_95038_.push("closing");
        AdvancedFontManager.this.fontSets.values().forEach(FontSet::close);
        AdvancedFontManager.this.fontSets.clear();
        p_95038_.popPush("reloading");
        p_95036_.forEach((p_95042_, p_95043_) -> {
           FontSet fontset = new FontSet(AdvancedFontManager.this.textureManager, p_95042_);
           fontset.reload(Lists.reverse(p_95043_));
           AdvancedFontManager.this.fontSets.put(p_95042_, fontset);
        });
        p_95038_.pop();
        p_95038_.endTick();
     }
...
public AdvancedFontManager(TextureManager p_95005_) {

...
public class ModifiedPackSelectionScreen extends Screen {

	@SuppressWarnings("unused")
	private static final int LIST_WIDTH = 200;
	private static final Component DRAG_AND_DROP = (new TranslatableComponent("pack.dropInfo"))
	...
	@Nullable
	private ModifiedPackSelectionScreen.Watcher watcher;
	...
	public ModifiedPackSelectionScreen(Screen p_99984_, PackRepository p_99985_, Consumer<PackRepository> p_99986_,
			File p_99987_, Component p_99988_) {
		super(p_99988_);
		...
		this.watcher = ModifiedPackSelectionScreen.Watcher.create(p_99987_);
	}
	...
	protected void init() {
		this.doneButton = this.addRenderableWidget(
				new Button(this.width / 2 + 4, this.height - 48, 150, 20, CommonComponents.GUI_DONE, (p_100036_) -> {
					this.onClose();
				}));
		this.addRenderableWidget(new Button(this.width / 2 - 154, this.height - 48, 150, 20,
				new TranslatableComponent("pack.openFolder"), (p_100004_) -> {
					...
					public void onTooltip(Button p_170019_, PoseStack p_170020_, int p_170021_, int p_170022_) {
						ModifiedPackSelectionScreen.this.renderTooltip(p_170020_,
								ModifiedPackSelectionScreen.DIRECTORY_BUTTON_TOOLTIP, p_170021_, p_170022_);
					}

					public void narrateTooltip(Consumer<Component> p_170017_) {
						p_170017_.accept(ModifiedPackSelectionScreen.DIRECTORY_BUTTON_TOOLTIP);
					}
				}));

		if (Client.getInstance().isDev()) // TODO: Make it for all
			this.addRenderableWidget(new Button(this.width / 2 - 154, this.height - 24, 150, 20,
					new TranslatableComponent("pack.open.store"), (p_100004_) -> {
						this.minecraft.setScreen(new PackStoreScreen(null, this));
					}));

		this.availablePackList = new TransferableSelectionList(this.minecraft, 200, this.height,
				new TranslatableComponent("pack.available.title"));
		this.availablePackList.setLeftPos(this.width / 2 - 4 - 200);
		this.addWidget(this.availablePackList);
		this.selectedPackList = new TransferableSelectionList(this.minecraft, 200, this.height,
				new TranslatableComponent("pack.selected.title"));
	...
	public void tick() {
		if (this.watcher != null) {
			try {
				...
				Client.getInstance().getLogger().warn(LoggingCategory.RESOURCE_PACK,
						"Failed to poll for directory " + packDir.getAbsolutePath() + " changes, stopping");
				this.closeWatcher();
			}
		}
	...
	private void updateList(TransferableSelectionList p_100014_, Stream<PackSelectionModel.Entry> p_100015_) {
		...
		p_100015_.filter(PackSelectionModel.Entry::notHidden).forEach((p_170000_) -> {
			p_100014_.children()
					.add(new TransferableSelectionList.PackEntry(this.minecraft, p_100014_, this, p_170000_));
		});
	}
	...
	protected static void copyPacks(Minecraft p_100000_, List<Path> p_100001_, Path p_100002_) {
		...
		p_100001_.forEach((p_170009_) -> {
			try {
				...
				try {
					stream.forEach((p_170005_) -> {
						try {
							...
							Client.getInstance().getLogger().warn(LoggingCategory.RESOURCE_PACK,
									"Failed to copy datapack file  from " + p_170005_ + " to " + p_100002_,
									ioexception1);
				...
				Client.getInstance().getLogger().warn(LoggingCategory.RESOURCE_PACK,
						"Failed to copy datapack file from " + p_170009_ + " to " + p_100002_);
				mutableboolean.setTrue();
			}
	...
	private ResourceLocation loadPackIcon(TextureManager p_100017_, Pack p_100018_) {
		try {
					...
					label86: {
						try {
							if (inputstream != null) {
								...
								ResourceLocation resourcelocation1 = new ResourceLocation("minecraft",
										"pack/" + Util.sanitizeName(s, ResourceLocation::validPathChar) + "/"
			...
			Client.getInstance().getLogger().warn(LoggingCategory.RESOURCE_PACK,
					"Failed to load icon from pack " + p_100018_.getId(), exception);
		}
	...
	static class Watcher implements AutoCloseable {
		...
		@Nullable
		public static ModifiedPackSelectionScreen.Watcher create(File p_100048_) {
			try {
				return new ModifiedPackSelectionScreen.Watcher(p_100048_);
			} catch (IOException ioexception) {
				Client.getInstance().getLogger().warn(LoggingCategory.RESOURCE_PACK,
						"Failed to initialize pack directory " + p_100048_ + " monitoring", ioexception);
				return null;
			}
		}

		private void watchDir(Path p_100050_) throws IOException {
			p_100050_.register(this.watcher, StandardWatchEventKinds.ENTRY_CREATE, StandardWatchEventKinds.ENTRY_DELETE,
		...
		public boolean pollForChanges() throws IOException {
			...
			while ((watchkey = this.watcher.poll()) != null) {
				for (WatchEvent<?> watchevent : watchkey.pollEvents()) {
					flag = true;
					if (watchkey.watchable() == this.packPath

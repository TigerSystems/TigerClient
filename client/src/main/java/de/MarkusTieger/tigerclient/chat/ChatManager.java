package de.MarkusTieger.tigerclient.chat;
...
public class ChatManager extends ChatComponent {
	...
	public void render(PoseStack p_93781_, int p_93782_) {
		if (!this.isChatHidden()) {
			...
			if (j > 0) {
				...
				double d3 = -8.0D * (this.minecraft.options.chatLineSpacing + 1.0D)
					...
					if (guimessage != null) {
						...
						if (j1 < 200 || flag) {
							...
							if (l1 > 3) {
								...
								p_93781_.translate(0.0D, 0.0D, 50.0D);
								this.minecraft.font.drawShadow(p_93781_, guimessage.getMessage(), 0.0F,
										(float) ((int) (d5 + d3)), 16777215 + (l1 << 24));
				...
				if (!this.chatQueue.isEmpty()) {
					...
					RenderSystem.enableBlend();
					p_93781_.translate(0.0D, 0.0D, 50.0D);
					this.minecraft.font.drawShadow(p_93781_,
	...
	private void addMessage(Component p_93788_, int p_93789_) {
		...
		Client.getInstance().getLogger().info(LoggingCategory.CHAT,
				"[CHAT] " + p_93788_.getString().replaceAll("\r", "\\\\r").replaceAll("\n", "\\\\n"));
	}
	...
	public boolean handleChatQueueClicked(double p_93773_, double p_93774_) {
		if (this.isChatFocused() && !this.minecraft.options.hideGui && !this.isChatHidden()
				&& !this.chatQueue.isEmpty()) {
			double d0 = p_93773_ - 2.0D;
			double d1 = (double) this.minecraft.getWindow().getGuiScaledHeight() - p_93774_ - 40.0D;
			if (d0 <= (double) Mth.floor((double) this.getWidth() / this.getScale()) && d1 < 0.0D
					&& d1 > (double) Mth.floor(-9.0D * this.getScale())) {
	...
	protected boolean isChatFocused() {
	...
	public int getHeight() {
		return getHeight((this.isChatFocused() ? this.minecraft.options.chatHeightFocused
				: this.minecraft.options.chatHeightUnfocused) / (this.minecraft.options.chatLineSpacing + 1.0D));
	}
